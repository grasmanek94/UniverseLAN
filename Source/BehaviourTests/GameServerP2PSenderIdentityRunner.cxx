...
bool traceHasExactResult(const fs::path& trace)
{
    ...
    return value == "{\"record\":\"direct-game-server-reply\",\"delivery\":true,\"senderType\":\"unclassified-by-public-sdk\",\"senderEqualsLobbyOwner\":true,\"senderEqualsSelf\":false,\"payloadMatchesPrivateMarker\":true}\n";
}
...
int run(const Arguments& arguments)
{
    ...
    {
        std::cerr << "BEHAVIOUR_TEST UNIVERSELAN scenario=game-server-p2p-sender-identity "
    ...
    const fs::path root = fs::temp_directory_path() / "universelan-behaviour-tests"
        / ("game-server-p2p-sender-identity-" + std::to_string(random()));
    ...
    try
    {
        ...
        std::cout << "BEHAVIOUR_TEST UNIVERSELAN scenario=game-server-p2p-sender-identity "
            "classification=characterized delivery=true senderType=unclassified-by-public-sdk senderRelation=lobby-owner" << std::endl;
        return 0;
    }
    catch (...)
    {
        ...
        std::error_code error;
        fs::remove_all(root, error);
        std::cerr << "BEHAVIOUR_TEST FAIL scenario=game-server-p2p-sender-identity" << std::endl;
        return 1;
    }
}
}
