...
virtual ~IErrorManager()
```
- Observed snapshot availability: 1.64.0, 1.66.0, 1.67.0, 1.68.0, 1.69.0, 1.70.0, 1.72.0, 1.73.0, 1.74.0, 1.75.0, 1.76.0, 1.77.0, 1.80.0, 1.87.0, 1.92.0, 1.95.0, 1.99.0, 1.100.2, 1.104.2, 1.104.3, 1.104.4, 1.106.0, 1.109.0, 1.112.2, 1.113.1, 1.113.3, 1.114.9.
- Relevant build matrix gate: GALAXY_BUILD_FEATURE_HAS_IERRORMANAGER.
...
1. Obtain the object only through its documented owner or accessor, exercise the adjacent public lifecycle, then release it only through that owner or documented shutdown path; do not call `~IErrorManager` directly.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: observe the smallest documented owner/accessor release lifecycle in both lanes, then add only the boundary observation needed to explain a difference.
