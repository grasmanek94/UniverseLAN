...
1. Supply a minimal instrumented public callback/factory with caller-owned state, invoke `GalaxyAllocator` only through its documented consumer, and record calls without retaining raw addresses or payloads.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: run the smallest valid `GalaxyAllocator` probe in both lanes, then add only the documented boundary or lifecycle observation needed to explain a difference.
