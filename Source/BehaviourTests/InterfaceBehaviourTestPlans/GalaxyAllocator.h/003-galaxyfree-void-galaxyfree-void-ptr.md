...
1. Define an instrumented `GalaxyFree` callback and supply it with matching `GalaxyMalloc` and `GalaxyRealloc` callbacks to `GalaxyAllocator`; consume that allocator through the documented `InitOptions` path, never by invoking the typedef as a standalone API.
