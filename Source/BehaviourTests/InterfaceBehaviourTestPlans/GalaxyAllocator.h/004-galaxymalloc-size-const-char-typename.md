...
1. Define an instrumented `GalaxyMalloc` callback and supply it with matching `GalaxyRealloc` and `GalaxyFree` callbacks to `GalaxyAllocator`; consume that allocator through the documented `InitOptions` path, never by invoking the typedef as a standalone API.
