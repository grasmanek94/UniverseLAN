# `static IChat* GALAXY_CALLTYPE Chat()`
...
static IChat* GALAXY_CALLTYPE Chat()
```
- Observed snapshot availability: 1.104.3, 1.104.4, 1.106.0, 1.109.0, 1.112.2, 1.113.1, 1.113.3, 1.114.9.
- Relevant build matrix gate: GALAXY_BUILD_FEATURE_HAS_ICHAT.
- Existing coverage: Grounded existing coverage: the chat scenarios reach `IChat` through the public GalaxyApi accessor.
...
- Lifecycle state before/after `Chat`, repeated-call characterization, public accessor availability, listener lifetime, and cleanup are recorded separately.
...
1. From the documented pre-state, invoke `Chat` with valid options/credentials supplied outside retained artifacts; immediately inspect only public error/state/accessor relations.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: run the smallest valid `Chat` probe in both lanes, then add only the documented boundary or lifecycle observation needed to explain a difference.
