# Macro expansion: `InvalidArgumentError` constructor
...
```cpp
`GALAXY_EXCEPTION_HELPER_ERROR_CLASS(InvalidArgumentError, IInvalidArgumentError, IError::INVALID_ARGUMENT)` expands a class containing `explicit InvalidArgumentError(const IError* exception)`.
```
...
1. Establish the documented prerequisite state in both lanes, then call `InvalidArgumentError` initially and after one valid public transition; record only its return/error and public state relation.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: run the smallest valid `InvalidArgumentError` probe in both lanes, then add only the documented boundary or lifecycle observation needed to explain a difference.
