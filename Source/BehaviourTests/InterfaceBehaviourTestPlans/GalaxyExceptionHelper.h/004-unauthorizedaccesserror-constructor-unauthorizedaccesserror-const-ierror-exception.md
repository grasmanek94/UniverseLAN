# Macro expansion: `UnauthorizedAccessError` constructor

## Declaration and applicability

- Header: `GalaxyExceptionHelper.h`
- Exact declaration/signature:
```cpp
`GALAXY_EXCEPTION_HELPER_ERROR_CLASS(UnauthorizedAccessError, IUnauthorizedAccessError, IError::UNAUTHORIZED_ACCESS)` expands a class containing `explicit UnauthorizedAccessError(const IError* exception)`.
```
...
- Relevant build matrix gate: GALAXY_BUILD_FEATURE_EXCEPTION_HELPER.
...
1. Establish the documented prerequisite state in both lanes, then call `UnauthorizedAccessError` initially and after one valid public transition; record only its return/error and public state relation.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: run the smallest valid `UnauthorizedAccessError` probe in both lanes, then add only the documented boundary or lifecycle observation needed to explain a difference.
