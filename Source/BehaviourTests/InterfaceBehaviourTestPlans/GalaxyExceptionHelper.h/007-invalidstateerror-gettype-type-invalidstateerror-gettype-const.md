# Macro-expanded `InvalidStateError::GetType()` member
...
```cpp
`GALAXY_EXCEPTION_HELPER_ERROR_CLASS(InvalidStateError, IInvalidStateError, IError::INVALID_STATE)` expands `virtual api::IError::Type GetType() const` inside `InvalidStateError`.
```
