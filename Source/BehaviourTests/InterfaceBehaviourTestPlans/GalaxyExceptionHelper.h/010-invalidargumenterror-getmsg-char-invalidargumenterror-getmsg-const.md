# Macro-expanded `InvalidArgumentError::GetMsg()` member
...
```cpp
`GALAXY_EXCEPTION_HELPER_ERROR_CLASS(InvalidArgumentError, IInvalidArgumentError, IError::INVALID_ARGUMENT)` expands `virtual const char* GetMsg() const` inside `InvalidArgumentError`.
```
