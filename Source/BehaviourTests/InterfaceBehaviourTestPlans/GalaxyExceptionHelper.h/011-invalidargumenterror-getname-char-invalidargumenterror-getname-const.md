# Macro-expanded `InvalidArgumentError::GetName()` member
...
```cpp
`GALAXY_EXCEPTION_HELPER_ERROR_CLASS(InvalidArgumentError, IInvalidArgumentError, IError::INVALID_ARGUMENT)` expands `virtual const char* GetName() const` inside `InvalidArgumentError`.
```
