# Macro-expanded `InvalidStateError::GetMsg()` member
...
```cpp
`GALAXY_EXCEPTION_HELPER_ERROR_CLASS(InvalidStateError, IInvalidStateError, IError::INVALID_STATE)` expands `virtual const char* GetMsg() const` inside `InvalidStateError`.
```
