# Macro-expanded `InvalidStateError::GetName()` member
...
```cpp
`GALAXY_EXCEPTION_HELPER_ERROR_CLASS(InvalidStateError, IInvalidStateError, IError::INVALID_STATE)` expands `virtual const char* GetName() const` inside `InvalidStateError`.
```
