# Macro-expanded `RuntimeError::GetMsg()` member
...
```cpp
`GALAXY_EXCEPTION_HELPER_ERROR_CLASS(RuntimeError, IRuntimeError, IError::RUNTIME_ERROR)` expands `virtual const char* GetMsg() const` inside `RuntimeError`.
```
