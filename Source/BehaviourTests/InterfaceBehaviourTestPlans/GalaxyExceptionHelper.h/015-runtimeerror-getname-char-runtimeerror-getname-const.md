# Macro-expanded `RuntimeError::GetName()` member
...
```cpp
`GALAXY_EXCEPTION_HELPER_ERROR_CLASS(RuntimeError, IRuntimeError, IError::RUNTIME_ERROR)` expands `virtual const char* GetName() const` inside `RuntimeError`.
```
