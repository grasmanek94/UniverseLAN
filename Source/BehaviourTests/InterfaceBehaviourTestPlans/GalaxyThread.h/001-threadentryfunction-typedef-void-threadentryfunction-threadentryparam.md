...
1. Implement an instrumented `IGalaxyThreadFactory::SpawnThread(ThreadEntryFunction const, ThreadEntryParam)` that observes the supplied `ThreadEntryFunction`; consume the factory through the documented `InitOptions` path rather than calling the callback type as an API.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: run the smallest `SpawnThread`/`InitOptions` consumer probe in both lanes, then add only the documented lifecycle observation needed to explain a difference.
