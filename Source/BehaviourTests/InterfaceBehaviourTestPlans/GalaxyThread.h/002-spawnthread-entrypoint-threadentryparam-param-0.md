...
- Relevant build matrix gate: GALAXY_BUILD_FEATURE_HAS_IGALAXYTHREADFACTORY.
...
1. Supply a minimal instrumented public callback/factory with caller-owned state, invoke `SpawnThread` only through its documented consumer, and record calls without retaining raw addresses or payloads.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: run the smallest valid `SpawnThread` probe in both lanes, then add only the documented boundary or lifecycle observation needed to explain a difference.
