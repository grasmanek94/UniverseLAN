...
1. Supply a minimal instrumented public callback/factory with caller-owned state, exercise only documented `Join` or `Detach` and owner release, and record lifecycle observations without retaining raw addresses or payloads.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: observe the smallest documented `Join`/`Detach` and owner-release lifecycle in both lanes, then add only the boundary observation needed to explain a difference.
