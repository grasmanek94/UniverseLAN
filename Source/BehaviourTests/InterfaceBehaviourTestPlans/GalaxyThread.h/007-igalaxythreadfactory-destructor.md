...
1. Supply a minimal instrumented public callback/factory with caller-owned state, exercise only the documented owner release lifecycle, and record observations without retaining raw addresses or payloads.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: observe the smallest documented factory-owner release lifecycle in both lanes, then add only the boundary observation needed to explain a difference.
