...
virtual void IsDlcOwned(ProductID productID, IIsDlcOwnedListener* const listener) = 0;
```
- Observed snapshot availability: 1.152.2, 1.152.6, 1.152.9, 1.152.11.
- Relevant build matrix gate: GALAXY_BUILD_FEATURE_HAS_IAPPS_ISDLCOWNED.
- Existing coverage: fixture-blocked. The public header requires a DLC `ProductID` and signed-in Galaxy Client; no valid title-neutral DLC fixture is available.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: provide one documented DLC product fixture with defined ownership in both lanes. Do not invent a product ID or dispatch the listener for an undocumented input.
