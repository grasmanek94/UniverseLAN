# `virtual void OnDlcCheckFailure(ProductID producId, FailureReason failueReason) = 0;`
...
virtual void OnDlcCheckFailure(ProductID producId, FailureReason failueReason) = 0;
```
- Observed snapshot availability: 1.152.2, 1.152.6.
- Relevant build matrix gate: GALAXY_BUILD_FEATURE_HAS_IAPPS.
- Existing coverage: fixture-blocked through `IsDlcOwned`; this older declaration is not present in the selected `1.152.11` header.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: use a documented DLC fixture when testing the matching historical SDK snapshot; do not infer selected-snapshot behavior from this declaration.
