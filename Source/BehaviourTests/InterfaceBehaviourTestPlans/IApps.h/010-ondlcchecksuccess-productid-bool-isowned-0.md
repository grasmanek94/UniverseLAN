...
virtual void OnDlcCheckSuccess(ProductID productId, bool isOwned) = 0;
```
- Observed snapshot availability: 1.152.9, 1.152.11.
- Relevant build matrix gate: GALAXY_BUILD_FEATURE_HAS_IAPPS.
- Existing coverage: fixture-blocked through `IsDlcOwned`. The public header requires a DLC `ProductID`; no valid title-neutral DLC fixture is available.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: provide one documented DLC product fixture with defined ownership in both lanes; retain only callback delivery and ownership relation, never the product ID.
