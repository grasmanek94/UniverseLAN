...
- Existing coverage: every `IApps` host probe obtains the interface through `Apps()` and uses SDK `Shutdown()`; direct destruction is intentionally not exercised.
...
1. Obtain the interface only through its documented accessor, exercise the adjacent public lifecycle, then use documented SDK shutdown; do not call `~IApps` directly.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: no direct destructor probe is valid. Continue to exercise ordinary `Apps()`/`Shutdown()` lifecycle only as part of public API scenarios.
