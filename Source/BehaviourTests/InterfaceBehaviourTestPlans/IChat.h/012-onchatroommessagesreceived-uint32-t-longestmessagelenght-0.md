...
- Existing coverage: No grounded existing coverage identified: `Source/BehaviourTests/COVERAGE.md` does not directly name `IChat::OnChatRoomMessagesReceived`.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: run the smallest valid `OnChatRoomMessagesReceived` probe in both lanes, then add only the documented boundary or lifecycle observation needed to explain a difference.
