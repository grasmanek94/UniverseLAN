...
typedef int (*ReadFunc)(void* userParam, char* data, int size);
```
- Observed snapshot availability: 1.148.7, 1.148.11, 1.148.14, 1.149.0, 1.150.0, 1.151.0, 1.152.1, 1.152.2, 1.152.6, 1.152.9, 1.152.11.
...
1. Provide an instrumented `ReadFunc` to the callback-source `ICloudStorage::PutFile` overload (and, where applicable, `CalculateHash`); observe callback arguments and return convention only through that consuming API.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: run the smallest `PutFile` callback-source consumer probe in both lanes, then add only the documented boundary or lifecycle observation needed to explain a difference.
