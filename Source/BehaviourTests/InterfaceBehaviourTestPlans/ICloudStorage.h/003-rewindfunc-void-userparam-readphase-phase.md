...
1. Provide an instrumented `RewindFunc` with `ReadFunc` to the callback-source `ICloudStorage::PutFile` overload (and, where applicable, `CalculateHash`); observe phase and return convention only through that consuming API.
