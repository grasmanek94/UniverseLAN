...
1. Provide an instrumented `WriteFunc` to the streaming `ICloudStorage::GetFile` overload; observe callback arguments and return convention only through that consuming API.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: run the smallest streaming `GetFile` consumer probe in both lanes, then add only the documented boundary or lifecycle observation needed to explain a difference.
