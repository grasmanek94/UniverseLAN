...
1. Establish the documented prerequisite state in both lanes, then call `GetFileSizeByIndex` initially and after one valid public transition; record only its return/error and public state relation.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: run the smallest valid `GetFileSizeByIndex` probe in both lanes, then add only the documented boundary or lifecycle observation needed to explain a difference.
