...
virtual void CalculateHash( void* userParam, ReadFunc readFunc, RewindFunc rewindFunc, char* hashBuffer, uint32_t hashBufferSize ) = 0;
```
- Observed snapshot availability: 1.149.0, 1.150.0, 1.151.0, 1.152.1, 1.152.2, 1.152.6, 1.152.9, 1.152.11.
- Relevant build matrix gate: GALAXY_BUILD_FEATURE_HAS_ICLOUDSTORAGE_HASHING.
...
1. Establish the documented prerequisite state in both lanes, then call `CalculateHash` initially and after one valid public transition; record only its return/error and public state relation.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: run the smallest valid `CalculateHash` probe in both lanes, then add only the documented boundary or lifecycle observation needed to explain a difference.
