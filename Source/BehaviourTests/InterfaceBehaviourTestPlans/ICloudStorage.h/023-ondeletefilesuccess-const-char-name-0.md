...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: run the smallest valid `OnDeleteFileSuccess` probe in both lanes, then add only the documented boundary or lifecycle observation needed to explain a difference.
