# `virtual void OnGetFileSuccess( const char* container, const char* name, uint32_t fileSize, uint32_t metadataCount ) = 0;`
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: run the smallest valid `OnGetFileSuccess` probe in both lanes, then add only the documented boundary or lifecycle observation needed to explain a difference.
