...
1. Obtain the interface only through its documented accessor, close public connections as documented, then use SDK shutdown; do not call `~ICustomNetworking` directly.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: observe the smallest documented accessor/close/shutdown lifecycle in both lanes, then add only the boundary observation needed to explain a difference.
