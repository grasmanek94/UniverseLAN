...
1. Establish self and explicit-peer rich-presence records where public setup permits, then call `GetRichPresence` with the default `GalaxyID()` and each explicit `GalaxyID`.
2. Query a known and an absent key separately for each target; record the returned public value and its documented lifetime without assuming the default-target relation.
3. Retain keys only for the call and complete ordinary shutdown cleanup.
