...
1. Establish self and explicit-peer rich-presence records where public setup permits, then call `GetRichPresenceCount` with the default `GalaxyID()` and each explicit `GalaxyID`.
2. Record counts before and after a documented public refresh without assuming the default-target relation or propagation timing.
