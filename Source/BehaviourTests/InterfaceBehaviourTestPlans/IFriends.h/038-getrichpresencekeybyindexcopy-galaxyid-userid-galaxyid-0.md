...
1. Establish self and explicit-peer rich-presence records where public setup permits, then call `GetRichPresenceKeyByIndexCopy` with the default `GalaxyID()` and each explicit `GalaxyID`.
2. Query in-range and out-of-range indexes with zero, exact, and undersized buffers separately for each target.
