# `virtual void OnRichPresenceUpdated(GalaxyID galaxyID) = 0;`
...
virtual void OnRichPresenceUpdated(GalaxyID galaxyID) = 0;
```
- Observed snapshot availability: 1.74.0, 1.75.0, 1.76.0, 1.77.0, 1.80.0, 1.87.0, 1.92.0, 1.95.0.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: run the smallest valid `OnRichPresenceUpdated` probe in both lanes, then add only the documented boundary or lifecycle observation needed to explain a difference.
