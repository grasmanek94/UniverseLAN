...
virtual void RequestRichPresence(GalaxyID userID = GalaxyID()) = 0;
```
- Observed snapshot availability: 1.74.0, 1.75.0, 1.76.0, 1.77.0, 1.80.0, 1.87.0, 1.92.0, 1.95.0, 1.99.0, 1.100.2, 1.104.2, 1.104.3, 1.104.4, 1.106.0, 1.109.0, 1.112.2, 1.113.1, 1.113.3, 1.114.9, 1.121.2, 1.124.0, 1.125.2.
...
1. Call `RequestRichPresence` once with the default `GalaxyID()` and once with an explicit peer `GalaxyID`; retain no assumption that the default selects self.
2. Pump the public processing API to a bounded deadline and record only documented public state or global-listener observations.
3. Treat delivery and default-target differences as characterization and complete ordinary shutdown cleanup.
