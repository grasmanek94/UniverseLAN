...
1. Call `RequestRichPresence` once omitting both default `GalaxyID()` and listener, then with an explicit peer `GalaxyID` and retained `IRichPresenceRetrieveListener`.
2. Pump the public processing API to a bounded deadline; compare terminal callback and public-state observations without assuming delivery or the default-target relation.
