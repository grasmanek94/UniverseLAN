...
- Lifecycle state before/after `~IGalaxy`, repeated-call characterization, public accessor availability, listener lifetime, and cleanup are recorded separately.
...
1. From the documented pre-state, obtain the instance through its documented factory and use only the factory reset/release lifecycle; inspect public state/accessor relations without directly calling `~IGalaxy`.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: observe the smallest documented factory reset/release lifecycle in both lanes, then add only the boundary observation needed to explain a difference.
