...
- Existing coverage: Grounded existing coverage: `Source/BehaviourTests/COVERAGE.md` describes scenarios using public `IMatchmaking::GetLobbyByIndex`.
...
1. Establish the documented prerequisite state in both lanes, then call `GetLobbyByIndex` initially and after one valid public transition; record only its return/error and public state relation.
...
- UniverseLAN server + 2 clients: `[x] Pass` `[ ] Fail`; 2026-09-16 focused lifecycle comparison observed callback-local in-range indexed candidates as valid lobby relations.
- Official GOG 2 clients: `[x] Pass` `[ ] Fail`; the same focused comparison matched the in-range callback-local indexed-candidate relation.
## Verdict and next step

- `[x] Pass` `[ ] Fail` `[ ] Not-run`
- Next step: repeat the official characterization before treating callback-local indexing behavior as stable across future SDK versions.
