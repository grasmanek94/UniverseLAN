...
- Existing coverage: Grounded existing coverage: `Source/BehaviourTests/COVERAGE.md` describes scenarios using public `IMatchmaking::GetLobbyMemberByIndex`.
...
1. Establish the documented prerequisite state in both lanes, then call `GetLobbyMemberByIndex` initially and after one valid public transition; record only its return/error and public state relation.
...
- UniverseLAN server + 2 clients: `[x] Pass` `[ ] Fail`; 2026-09-16 focused lifecycle comparison observed valid, distinct in-range members with stable repeated reads.
- Official GOG 2 clients: `[x] Pass` `[ ] Fail`; the same focused comparison matched the in-range member relations, including the local-member-at-index-zero observation.
## Verdict and next step

- `[x] Pass` `[ ] Fail` `[ ] Not-run`
- Next step: repeat the official characterization before treating the local-member index relation as stable across future SDK versions.
