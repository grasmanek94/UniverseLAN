...
- Existing coverage: `Simple/public-lobby-create-list-join-leave` performs in-range member-data enumeration and matched a focused four-host run on 2026-09-16.
...
1. Establish the documented prerequisite state in both lanes, then call `GetLobbyMemberDataByIndex` initially and after one valid public transition; record only its return/error and public state relation.
...
- UniverseLAN server + 2 clients: `[x] Pass` `[ ] Fail`; in-range public member-data reads succeeded and found the temporary expected relation without retaining key/value data.
- Official GOG 2 clients: `[x] Pass` `[ ] Fail`; the same focused comparison matched all in-range member-data read relations.
## Verdict and next step

- `[x] Pass` `[ ] Fail` `[ ] Not-run`
- Next step: repeat the official characterization before extending this in-range observation to buffer-boundary behavior.
