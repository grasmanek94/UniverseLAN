...
- Existing coverage: Grounded existing coverage: `Source/BehaviourTests/COVERAGE.md` describes scenarios using public `IMatchmaking::GetLobbyMessage`.
...
2. At availability/callback, perform only the read/peek/pop form declared by `GetLobbyMessage`, using documented exact/undersized boundaries. Record success, symbolic sender/channel/message relation, and whether a subsequent availability query changes.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: run the smallest valid `GetLobbyMessage` probe in both lanes, then add only the documented boundary or lifecycle observation needed to explain a difference.
