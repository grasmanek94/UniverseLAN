...
virtual void AddRequestLobbyListResultCountFilter(uint32_t limit) = 0;
```
- Observed snapshot availability: 1.131.3, 1.132.1, 1.133.0, 1.133.6, 1.134.8, 1.134.9, 1.134.10, 1.135.0, 1.138.0, 1.139.2, 1.139.5, 1.139.6, 1.139.9, 1.140.0, 1.142.0, 1.144.1, 1.148.1, 1.148.2, 1.148.3, 1.148.5, 1.148.6, 1.148.7, 1.148.11, 1.148.14, 1.149.0, 1.150.0, 1.151.0, 1.152.1, 1.152.2, 1.152.6, 1.152.9, 1.152.11.
- Relevant build matrix gate: GALAXY_BUILD_FEATURE_HAS_IMATCHMAKING_LOBBY_LIST_RESULT.
...
- Observe public state before and after `AddRequestLobbyListResultCountFilter`, one repeated/idempotency characterization, and cleanup/reversal only where the API exposes it.
...
1. Establish a legal public target, snapshot the relevant public pre-state, invoke `AddRequestLobbyListResultCountFilter` once, and record its immediate result/error plus any documented terminal listener.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: run the smallest valid `AddRequestLobbyListResultCountFilter` probe in both lanes, then add only the documented boundary or lifecycle observation needed to explain a difference.
