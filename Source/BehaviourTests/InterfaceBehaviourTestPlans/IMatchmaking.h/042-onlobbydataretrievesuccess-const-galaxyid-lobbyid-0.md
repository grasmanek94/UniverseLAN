...
- Relevant build matrix gate: GALAXY_BUILD_FEATURE_HAS_ILOBBYDATARETRIEVELISTENER.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: run the smallest valid `OnLobbyDataRetrieveSuccess` probe in both lanes, then add only the documented boundary or lifecycle observation needed to explain a difference.
