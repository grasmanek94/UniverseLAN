...
1. Obtain the interface only through its documented accessor, leave temporary public lobbies, then use SDK shutdown; do not call `~IMatchmaking` directly.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: observe the smallest documented accessor/leave/shutdown lifecycle in both lanes, then add only the boundary observation needed to explain a difference.
