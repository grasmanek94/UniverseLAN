...
virtual bool SendP2PPacket(GalaxyID galaxyID, const void* data, uint32_t dataLen, P2PSendType sendType, uint8_t channel = 0) = 0;
```
- Observed snapshot availability: 1.57.0, 1.60.0, 1.61.0, 1.64.0, 1.66.0, 1.67.0, 1.68.0, 1.69.0, 1.70.0, 1.72.0, 1.73.0, 1.74.0, 1.75.0, 1.76.0, 1.77.0, 1.80.0, 1.87.0.
- Relevant build matrix gate: No dedicated feature gate; compile only where this exact declaration is present.
- Existing coverage: Grounded existing coverage: `Source/BehaviourTests/COVERAGE.md` describes scenarios using public `INetworking::SendP2PPacket`.
...
- Observe public state before and after `SendP2PPacket`, one repeated/idempotency characterization, and cleanup/reversal only where the API exposes it.
...
1. Establish a legal public target, snapshot the relevant public pre-state, invoke `SendP2PPacket` once, and record its immediate result/error plus any documented terminal listener.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: run the smallest valid `SendP2PPacket` probe in both lanes, then add only the documented boundary or lifecycle observation needed to explain a difference.
