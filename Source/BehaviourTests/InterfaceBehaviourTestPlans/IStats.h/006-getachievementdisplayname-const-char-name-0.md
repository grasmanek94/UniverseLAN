...
1. Establish the documented prerequisite state in both lanes, then call `GetAchievementDisplayName` initially and after one valid public transition; record only its return/error and public state relation.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: run the smallest valid `GetAchievementDisplayName` probe in both lanes, then add only the documented boundary or lifecycle observation needed to explain a difference.
