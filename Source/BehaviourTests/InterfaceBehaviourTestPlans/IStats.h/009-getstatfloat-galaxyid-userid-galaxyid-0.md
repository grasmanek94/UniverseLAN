...
1. Call `GetStatFloat` for a known name with the default `GalaxyID()` and with explicit self/peer `GalaxyID` values where public setup permits.
2. Record query results before and after the documented retrieval path; characterize absent-name and default-target behavior separately.
3. Complete ordinary shutdown cleanup without retaining runtime stat values.
