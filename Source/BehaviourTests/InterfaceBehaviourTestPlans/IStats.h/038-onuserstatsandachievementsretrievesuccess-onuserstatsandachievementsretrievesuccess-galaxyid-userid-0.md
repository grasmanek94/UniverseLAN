...
- Existing coverage: `stats-retrieve-self-callback` strict two-lane CTest verifies successful retrieval and callback-user-equals-public-self; `stats-retrieve-achievements-number characterization` also records terminal/self relation without retaining catalog data. See [header coverage](HEADER_COVERAGE.md).
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: run the smallest valid `OnUserStatsAndAchievementsRetrieveSuccess` probe in both lanes, then add only the documented boundary or lifecycle observation needed to explain a difference.
