...
1. Call `RequestLeaderboardEntriesAroundUser` once omitting both default `GalaxyID()` and listener, then with an explicit `GalaxyID` and retained `ILeaderboardEntriesRetrieveListener`.
2. Pump the public processing API to a bounded deadline; compare terminal callback and queryable entry observations without assuming delivery or the default-target relation.
