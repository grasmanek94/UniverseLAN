...
1. Call `RequestLeaderboardEntriesAroundUser` with the default `GalaxyID()` and with explicit self/peer `GalaxyID` values where public setup permits.
2. Pump the public processing API to a bounded deadline and record documented global-listener or queryable entry observations separately from delivery characterization.
