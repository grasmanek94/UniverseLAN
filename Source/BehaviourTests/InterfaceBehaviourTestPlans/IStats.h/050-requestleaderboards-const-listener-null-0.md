...
- Existing coverage: runtime-blocked in a 2026-09-16 two-lane terminal-only request attempt, which exited before trace output. No behavior result is classified. See [header coverage](HEADER_COVERAGE.md).
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: resolve the pre-trace runtime exit with an independently isolated request form. Do not classify a difference or modify implementation code from the blocked attempt.
