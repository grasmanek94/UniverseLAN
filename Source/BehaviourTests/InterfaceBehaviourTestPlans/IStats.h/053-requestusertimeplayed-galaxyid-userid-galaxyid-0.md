...
1. Call `RequestUserTimePlayed` with the default `GalaxyID()` and with explicit self/peer `GalaxyID` values where public setup permits.
2. Pump the public processing API to a bounded deadline and record documented terminal state or global-listener observations separately from delivery characterization.
