...
- Existing coverage: runtime-blocked in a 2026-09-16 two-lane default-self request attempt, which exited before trace output. No behavior result is classified. See [header coverage](HEADER_COVERAGE.md).
...
1. Call `RequestUserTimePlayed` once omitting both default `GalaxyID()` and listener, then with an explicit `GalaxyID` and retained `IUserTimePlayedRetrieveListener`.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: resolve the pre-trace runtime exit with an independently isolated request form. Do not call `GetUserTimePlayed` before a successful callback or classify a difference from the blocked attempt.
