...
- Observe public state before and after `StoreStatsAndAchievements`, one repeated/idempotency characterization, and cleanup/reversal only where the API exposes it.
...
1. Establish a legal public target, snapshot the relevant public pre-state, invoke `StoreStatsAndAchievements` once, and record its immediate result/error plus any documented terminal listener.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: run the smallest valid `StoreStatsAndAchievements` probe in both lanes, then add only the documented boundary or lifecycle observation needed to explain a difference.
