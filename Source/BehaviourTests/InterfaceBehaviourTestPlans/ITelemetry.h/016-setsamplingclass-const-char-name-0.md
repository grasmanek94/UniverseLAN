...
virtual void SetSamplingClass(const char* name) = 0;
```
- Observed snapshot availability: 1.142.0, 1.144.1, 1.148.1, 1.148.2, 1.148.3, 1.148.5, 1.148.6, 1.148.7, 1.148.11, 1.148.14, 1.149.0, 1.150.0, 1.151.0, 1.152.1, 1.152.2, 1.152.6, 1.152.9, 1.152.11.
- Relevant build matrix gate: GALAXY_BUILD_FEATURE_HAS_ITELEMETRY_SAMPLING_CLASS.
...
- Observe public state before and after `SetSamplingClass`, one repeated/idempotency characterization, and cleanup/reversal only where the API exposes it.
...
1. Establish a legal public target, snapshot the relevant public pre-state, invoke `SetSamplingClass` once, and record its immediate result/error plus any documented terminal listener.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: run the smallest valid `SetSamplingClass` probe in both lanes, then add only the documented boundary or lifecycle observation needed to explain a difference.
