...
1. Establish self and explicit-peer user-data records where public setup permits, then call `GetUserDataByIndex` with the default `GalaxyID()` and each explicit `GalaxyID`.
2. Query index and key/value buffer boundaries separately for each target; record copied fields without assuming the undocumented default-target relation.
