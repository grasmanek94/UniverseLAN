...
virtual const char* GetIDToken() = 0;
```
- Observed snapshot availability: 1.152.1, 1.152.2, 1.152.6, 1.152.9, 1.152.11.
- Relevant build matrix gate: GALAXY_BUILD_FEATURE_HAS_IUSER_AUTHORIZATION_IDTOKEN.
...
1. Establish the documented prerequisite state in both lanes, then call `GetIDToken` initially and after one valid public transition; record only its return/error and public state relation.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: run the smallest valid `GetIDToken` probe in both lanes, then add only the documented boundary or lifecycle observation needed to explain a difference.
