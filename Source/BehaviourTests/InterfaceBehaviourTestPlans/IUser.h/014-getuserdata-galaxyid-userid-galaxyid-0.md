...
1. Establish self and explicit-peer user-data records where public setup permits, then call `GetUserData` with the default `GalaxyID()` and each explicit `GalaxyID`.
2. Query a known and an absent key separately for each target; record the returned public value and documented lifetime without assuming the default-target relation.
