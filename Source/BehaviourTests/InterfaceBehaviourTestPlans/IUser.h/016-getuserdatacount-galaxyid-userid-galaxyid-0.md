...
1. Establish self and explicit-peer user-data records where public setup permits, then call `GetUserDataCount` with the default `GalaxyID()` and each explicit `GalaxyID`.
2. Record counts before and after the documented retrieval path without assuming the default-target relation or propagation timing.
