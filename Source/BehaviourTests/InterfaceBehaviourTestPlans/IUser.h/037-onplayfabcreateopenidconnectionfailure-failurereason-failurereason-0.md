...
virtual void OnPlayFabCreateOpenIDConnectionFailure(FailureReason failureReason) = 0;
```
- Observed snapshot availability: 1.152.6, 1.152.9, 1.152.11.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: run the smallest valid `OnPlayFabCreateOpenIDConnectionFailure` probe in both lanes, then add only the documented boundary or lifecycle observation needed to explain a difference.
