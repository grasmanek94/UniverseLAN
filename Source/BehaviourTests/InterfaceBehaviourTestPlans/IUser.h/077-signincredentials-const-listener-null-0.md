...
- Existing coverage: Grounded existing coverage: `Source/BehaviourTests/COVERAGE.md` describes scenarios using public `IUser::SignInCredentials`.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: run the smallest valid `SignInCredentials` probe in both lanes, then add only the documented boundary or lifecycle observation needed to explain a difference.
