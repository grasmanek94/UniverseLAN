...
virtual void SignInLauncher(IAuthListener* const listener = NULL) = 0;
```
- Observed snapshot availability: 1.148.1, 1.148.2, 1.148.3, 1.148.5, 1.148.6, 1.148.7, 1.148.11, 1.148.14, 1.149.0, 1.150.0, 1.151.0, 1.152.1, 1.152.2, 1.152.6, 1.152.9, 1.152.11.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: run the smallest valid `SignInLauncher` probe in both lanes, then add only the documented boundary or lifecycle observation needed to explain a difference.
