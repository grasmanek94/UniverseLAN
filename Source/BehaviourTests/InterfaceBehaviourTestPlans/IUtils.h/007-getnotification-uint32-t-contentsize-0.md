# `virtual uint32_t GetNotification(uint32_t notificationID, char* type, uint32_t typeLength, void* content, uint32_t contentSize) = 0;`
...
virtual uint32_t GetNotification(uint32_t notificationID, char* type, uint32_t typeLength, void* content, uint32_t contentSize) = 0;
```
- Observed snapshot availability: 1.92.0.
...
1. Register the notification type through `RegisterForNotification`, connect the version-matching notification listener, and retain its `OnNotificationReceived(uint32_t, typeLength, contentSize)` callback ID and lengths.
2. In that callback, call this exact `GetNotification(uint32_t, char*, uint32_t, void*, uint32_t contentSize)` overload with buffers related to callback `typeLength` and `contentSize`; record copied lengths.
