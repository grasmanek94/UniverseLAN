# `virtual void OnNotificationReceived(uint32_t notificationID, uint32_t typeLength, uint32_t contentSize) = 0;`
...
1. Register a type through `RegisterForNotification` and connect the version-matching notification listener; on `OnNotificationReceived(uint32_t, typeLength, contentSize)`, retain only the ID and lengths.
2. Consume that callback ID immediately with the matching `GetNotification(uint32_t, ..., contentSize)` overload and buffers sized from the callback relation.
