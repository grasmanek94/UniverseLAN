...
1. Construct `ShutdownOptions` with defaults and with one documented explicit field at a time; inspect only public fields/value semantics exposed by the struct or class.
...
- `[ ] Pass` `[ ] Fail` `[x] Not-run`
- Least-feasible next step: run the smallest valid `ShutdownOptions` probe in both lanes, then add only the documented boundary or lifecycle observation needed to explain a difference.
