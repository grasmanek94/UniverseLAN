...
namespace galaxy
{
	namespace api
	{
		 ...
		 *
		 * Error occurred on call to Galaxy API can be obtain by calling GetError(),
		 ...
		 *
		 * To receive the packets that come to your Galaxy Peer get the instance
		 * of INetworking by calling GetNetworking().
		 *
		 ...
		 * @section chat Chat
		 *
		 * The Galaxy API allows to embed the GOG Galaxy chat, available for the GOG users on the
		 * [website](https://www.gog.com/account/chat), in the GOG Galaxy Client and in the in-game overlay,
		 * directly into the game.
		 *
		 * @subsection chat-room Chat room
		 *
		 * A chat room with a user can be created explicitly by calling IChat::RequestChatRoomWithUser(),
		 * or implicitly by new messages incoming to the IChatRoomMessagesListener::OnChatRoomMessagesReceived()
		 * callback listener.
		 * In this chat room, you can get the number of unread messages, by calling IChat::GetChatRoomUnreadMessageCount(),
		 * or iterate over chat room participants by calling IChat::GetChatRoomMemberUserIDByIndex(),
		 * up to the IChat::GetChatRoomMemberCount() participants.
		 *
		 * @subsection chat-history Chat history
		 *
		 * If you need to retrieve the chat room history, call IChat::RequestChatRoomMessages() method.
		 * By omitting the optional parameter \p referenceMessageID, only the latest messages will be retrieved.
		 * Retrieved messages will be available in the IChatRoomMessagesListener::OnChatRoomMessagesReceived() call,
		 * during which they can be read by calling IChat::GetChatRoomMessageByIndex().
		 * Once all messages has been processed and rendered to the user, they can be marked as read
		 * by calling the IChat::MarkChatRoomAsRead() method.
		 *
		 * @subsection chat-messaging Sending and receiving chat messages
		 *
		 * Sending messages is possible only to the existing (see above) chat rooms by calling IChat::SendChatRoomMessage().
		 * Every participant of a chat room will receive messages coming to the IChatRoomMessagesListener::OnChatRoomMessagesReceived() callback method.
		 *
		 ...
		 * was created and before it was initialized.
		 *
		 * @pre Delete all self-registering listeners before calling ShutdownEx().
		 *
		 * @param [in] shutdownOptions The group of the shutdown options.
		 */
		 ...
		 *
		 * @return An instance of ICloudStorage.
		 */
