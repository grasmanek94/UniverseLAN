...
namespace galaxy
{
	namespace api
	{
		...
		{
			GalaxyFactory::GetInstance()->Init(clientID, clientSecret, throwExceptions);
		}
		...
		{
			GalaxyFactory::GetInstance()->InitLocal(clientID, clientSecret, galaxyPeerPath, throwExceptions);
		}
		...
		{
			return GalaxyFactory::GetInstance()->GetError();
		}
