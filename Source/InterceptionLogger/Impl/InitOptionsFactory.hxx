...
namespace galaxy
{
	namespace api
	{
		...
		struct InitOptionsFactory
		{
			 ...
			 * @param [in] throwExceptions If exceptions should be thrown on errors (not yet implemented).
			 */
			InitOptionsFactory(const char* _clientID, const char* _clientSecret, const char* _configFilePath = ".", void* _unused = nullptr, const char* _storagePath = nullptr, const bool throwExceptions = true)
				: clientID(_clientID)
